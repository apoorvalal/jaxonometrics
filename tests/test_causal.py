...
def generate_causal_data(
    ...
    W_noise = jax.random.uniform(key_T_noise, (n_samples,))
    W = (W_noise < true_propensity).astype(jnp.int32)

    # True coefficients for outcome model (linear)
    beta_Y_common = jax.random.uniform(
        key_beta_Y, (n_features,), minval=-0.5, maxval=0.5
    )
    ...
    y = W * true_mu1 + (1 - W) * true_mu0 + Y_noise
    ...
    return X_intercept, W, y, true_propensity, true_mu0, true_mu1, true_ate
...
def test_ipw_ate_estimation(causal_sim_data):
    X, W, y, _, _, _, true_ate = causal_sim_data

    ipw_estimator = IPW(
        propensity_maxiter=10000
    )  # Changed logit_maxiter to propensity_maxiter
    ...
    np.testing.assert_allclose(
        estimated_ate, true_ate, rtol=0.2, atol=0.2
    )  # Looser tolerance for IPW
...
def test_aipw_ate_estimation(causal_sim_data):
    ...
    assert estimated_ate is not None
    np.testing.assert_allclose(
        estimated_ate, true_ate, rtol=0.15, atol=0.15
    )  # Potentially tighter tolerance for AIPW
    ...
    assert aipw_estimator.params["mu0_params"] is not None or X[W == 0].shape[0] == 0
    assert aipw_estimator.params["mu1_params"] is not None or X[W == 1].shape[0] == 0
...
def test_aipw_with_custom_models(causal_sim_data):
    ...
    ps_model.fit(X, W)  # X includes intercept
    ...
    aipw_estimator = AIPW(
        ...
        propensity_model=LogisticRegression(
            maxiter=10000
        ),  # It will create a new instance and fit
    )
    aipw_estimator.fit(X, W, y)
    estimated_ate = aipw_estimator.params["ate"]
