...
from sklearn.datasets import make_classification, make_regression # make_regression can simulate poisson data
from sklearn.preprocessing import StandardScaler

from jaxonometrics.mle import Logit, PoissonRegression

# Fixture for Logit data
@pytest.fixture
def logit_data():
    X_np, y_np = make_classification(n_samples=200, n_features=3, n_informative=2, n_redundant=1, random_state=42, n_classes=2)
    # Add an intercept column
    X_np_intercept = np.hstack([np.ones((X_np.shape[0], 1)), X_np])
    scaler = StandardScaler()
    X_np_scaled = scaler.fit_transform(X_np_intercept[:, 1:]) # Scale only non-intercept features
...
@pytest.fixture
def poisson_data():
    X_np, y_np_reg = make_regression(n_samples=200, n_features=3, n_informative=2, n_targets=1, random_state=123, noise=0.1)
    # Add an intercept column
    X_np_intercept = np.hstack([np.ones((X_np.shape[0], 1)), X_np])
    scaler = StandardScaler()
    ...
    y_poisson_np = np.abs(np.round(np.exp(y_np_reg / np.std(y_np_reg) * 0.5))) # Scale and transform
...
def test_logit_fit_predict(logit_data):
    ...
    # Jaxonometrics Logit
    jax_logit = Logit(maxiter=10000) # Increased maxiter for better convergence
    jax_logit.fit(X, y)
    ...
    sklearn_logit = SklearnLogit(solver='liblinear', C=1e9, fit_intercept=False, random_state=42, tol=1e-6)
...
def test_poisson_fit_predict(poisson_data):
    ...
    jax_poisson = PoissonRegression(maxiter=10000) # Increased maxiter
    jax_poisson.fit(X, y)
    ...
    sklearn_poisson = SklearnPoisson(alpha=0, fit_intercept=False, max_iter=1000, tol=1e-6)
